...
ggplot(data=sub, aes(x=conf, y= prop)) + geom_point() + labs(title="Accuracy versus Confidence", y = "Accuracy", x = "Average Confidence")
@
  \label{confidence2}
...
Turka <- Turk[c(2, 10, 11, 13, 17, 20)]
pin <- merge(counts, Turka, by.x="lineup", by.y="pic_id")
pin$r <- factor(pin$r)
head(pin)
colnames(pin) <- c("lineup", "plot", "count", "smoothed", "true_pos", "n", "design", "r")
pin$base <- 0
pin <- read.csv("pin.csv") #Do this part through R, not excel
...
m8l <- glmer(correct~offset(alpha) + r + rsgn + r*rsgn + smoothed + n + (1|subj_id), data=subset(full.sub, plot == "L"), family="binomial")
m8s <- glmer(correct~offset(alpha) + r + rsgn + r*rsgn + smoothed + n + (1|subj_id), data=subset(full.sub, plot == "S"), family="binomial")
@
