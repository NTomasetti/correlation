...
\begin{figure}[htbp]
\centering 
<<avscore, dependson='setup' =, echo=FALSE, results='asis', message=FALSE, error=FALSE, warning=FALSE>>=LSE>>=  
...
\begin{table}
\caption{Number of selection per lineup evaluation. The subject with the most selections is also reported.}
\label{tabattempts}
