...
#sasadata <- mutate(sasdata, rabs = as.numeric(rabs),
#                   subj_id = as.numeric(subj_id)
#                   design = ifelse(design=="Scatterplot", 1, 0))
#write.csv(subsetted, "sas/sasdata.csv", row.names = FALSE)
...
If all $K$ observers pick the same plot out of a lineup of twenty, we result in a p-value as small as $1/m^K$ . We used $m= 20$, giving us a significance level (and Type 1 error rate) of $\alpha = 0.05$. The plot of 'true' data is placed randomly amongst 19 null plots to form the lineup.
...
Subjects were allowed to make multiple selections from each lineup evaluated, additional selections on lineup $i$ by subject $j$ are penalised according to a score defined by:
\begin{equation}
\label{score}
...
However, for the model fit to power, multiple selections are seperated and given a weighting equal to to inverse of the number of selections made in that lineup evaluation. This increases our data from 1684 to 1849 evaluations.
...
predplot <- mutate(prediction, RandomEf = exp(RandomEf)/(1+exp(RandomEf)), 
