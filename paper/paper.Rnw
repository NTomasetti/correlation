...
full <- select(full, subj_id, correct, score, r, rsgn, rabs, smoothed, n, design, log_time, response_0, actual_location, pic_id, choice_reason, attempts, weights, alpha)
...
prop <- Turk[order(Turk$r, Turk$n, Turk$smoothed),c("smoothed", "n", "attempt", "correct", "r", "design")]
prop <- summarise(group_by(prop, smoothed, n, r), l.attempt=sum(ifelse(design=="Overlaid Lines", attempt, 0)),
                                                  l.correct=sum(ifelse(design=="Overlaid Lines", correct, 0)),
                                                  s.attempt=sum(ifelse(design=="Scatterplot", attempt, 0)),
                                                  s.correct=sum(ifelse(design=="Scatterplot", correct, 0)))
...
time <- select(full, design, log_time, correct,, sign)
...
conf <- select(full, design, conf_level, correct, sign)
...
conf2 <- select(full, conf_level, correlation, smoothed, n, design) #c(7, 15
...
prediction <- select(prediction, r, t, rsgn, smoothed, design, subj_id, RandomEf, FixedEf)
prediction <- mutate(prediction, RandomEf = exp(RandomEf)/(1+exp(RandomEf)), FixedEf = exp(FixedEf)/(1+exp(FixedEf)))
 
qplot(r, RandomEf, data=prediction, color=design, 
...
\label{tab2}
\centering
<<timefit, dependson='setup', results='asis', echo=FALSE, message=FALSE, error=FALSE, error=FALSE, message=FALSE, warning=FALSE>>=
...
Table~\ref{tab3} contains the true plot detections / attempts for each lineup in the data collection. Results ignore multiple selection weighting. True plot correlation is on the x axis and t is on the y axis. 
