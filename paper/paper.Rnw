...
\begin{figure}[htbp]
  \centering
  \begin{subfigure}
    \includegraphics{nzdscatter}
  \end{subfigure}
  \begin{subfigure}
    \includegraphics{nzdline}
  \end{subfigure}
...
full.sub <- select(full.sub, correct, r, rsgn, smoothed, n, subj_id, plot, log_time)
...
timelmer <-  lmer(log_time ~ plot*smoothed + plot*n + plot*rsgn*r + plot*correct + (1|subj_id), data=full.sub)
@
%get table code
