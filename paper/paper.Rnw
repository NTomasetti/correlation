...
@
  \label{results}
  \caption{Response accuracy for unsmoothed data, broken by unsmoothed (left), smoothed (right) and sample size. The dots are data points, the lines represent a naive model average fit. Both plot designs have similar performance for positively correlated data, however the overlaid line graphs are particularly weak at revealing negative correlation.}
\end{figure}
...
\begin{figure}[htbp]
  \centering
<<powerdiff, dependson='results', fig.width=6, fig.height=8, out.width='0.7\\textwidth', echo=FALSE, error=FALSE, message=FALSE, warning=FALSE>>=
...
@
 \label{powerdiff}
 \caption{The difference in estimated power (scatter plot power minus overlaid line power) to reveal the true plot by smoothness and sample size with a 95\% confidence interval. The red line indicates the point where both plot designs are equally powerful while values above this line indicate that the scatter plot is more powerful than the overlaid line graph. Again, the largest advantage difference appears in negative correlation, where the scatter plot is more powerful.}
\end{figure}

\begin{figure}[htbp]
  \centering
<<samplesize, dependson='powerdiff', fig.width=6, fig.height=8, out.width='0.7\\textwidth', echo=FALSE, error=FALSE, message=FALSE, warning=FALSE>>=
...
@
 \label{samplesize}
 \caption{Replotting figure~\ref{results} to highlight the effects of changes in sample size of unsmoothed data. The scatter plot appears to universaly benefit from increased sample sizes but the overlaid lines only see a benefit in positive correlation.}
\end{figure}
...
\begin{figure}[htbp]
  \centering
<<confidence, dependson='time', fig.width=6, fig.height=6, out.width='0.7\\textwidth', echo=FALSE, error=FALSE, message=FALSE, warning=FALSE>>=
...
\begin{figure}[htbp]
  \centering
<<confidence2, dependson='time', fig.width=6, fig.height=6, out.width='0.7\\textwidth', echo=FALSE, error=FALSE, message=FALSE, warning=FALSE>>=
...
@
  \label{confidence2}
  \caption{Individual subject accuracy versus subject average confidence. There does not appear to be any relationship between the two.}
\end{figure}
...
\begin{figure}[htbp]
  \centering
<<pins, dependson='samplesize', fig.width=6, fig.height=10, out.width='0.7\\textwidth', echo=FALSE, error=FALSE, message=FALSE, warning=FALSE>>=
...
colnames(counts) <- c("lineup", "plots", "count")
pin <- merge(counts, Turk[-17], by.x="lineup", by.y="pic_id")
pin <- select(pin, lineup, plots, count, smoothed, true_pos, n, test_param, r, attempt)#pin[,c(1, 2, 3, 12, 13, 15, 11, 20)]
pin$r <- factor(pin$r)
colnames(pin) <- c("lineup", "plot", "count", "smoothed", "true_pos", "n", "design", "r", "attempt")
...
$$Power_{ijklt} = \psi\left\{ \beta_{0} + \beta_{1} x_{1,i}  + \beta_{2} x_{2,j}  + \beta_{3} x_{3,k} + \beta_{4} x_{4,t} + \beta_{5} x_{1,i} \* x_{2,j} + \beta_{6} x_{5,l}(\beta_{7} + \beta_{8} x_{1,i}  + \beta_{9} x_{2,j}  + \beta_{10} x_{3,k} + \beta_{11} x_{4,t} + \beta_{12} x_{1,i} \* x_{2,j}) \gamma z_{s} + \epsilon_{ijkt} \right\}$$

Where $\psi$ is the logit link function, $\psi(x) = \frac{e^{x}}{1+e^{x}}$, $x_{1}$ is the absolute value of the true plot correlation, $\rho$, with $i \in {0.3, 0.5, 0.7, 0.9}$, $x_{2,j}$ is a dummy variable for the sign of the true plot correlation, with $j = 0$ for negative correlation, $x_{3,k}$ is a dummy variable for the smoothing procedure, with $k = 0$ for unsmoothed data, $x_{4,t}$ is the sample size, with $t \in {12, 24, 48, 96}$ and $x_{5,l}$ denotes the plot design, with $l =0$ for the overlaid lines type. Finally, $z_{s}$ is an observer specific random effect, with $s = 1, 2, \cdot , 157$. It is assumed that $\epsilon_{ijkt} \sim \mathcal{N} (0, \Sigma_{ijkt})$ and $\gamma \sim \mathcal{N} (0, \delta)$

Where a subject made multiple selections, each selection is treated as a separate data point with a probability weighting equal to the inverse of the number of selections.

\begin{table}[htp]
\caption{Weighted Mixed Effects Logit Model for the predicted power of plot design. Sample size for scatter plot evaluations: 900, overlaid lines: 949, total: 1849. Default plot design is overlaid lines. The model is fitted using the GLIMMIX procedure in SAS.} %Sign = 1 if correlation > 0, 0 otherwise. Each plot design is estimated separately. Results must be transformed via the logit link function to generate a power prediction. Subjects with less than ten lineup evaluations were removed from the sample.

\label{tab1}
\centering
<<powerfit, dependson='setup', results='asis', echo=FALSE, message=FALSE, error=FALSE, error=FALSE, message=FALSE, warning=FALSE>>=
full$subj_id <- factor(full$ip_address, labels=1:207)
...
full.subs <- filter(full, subj_id %in% keep)
full.subs$r <- round(full.subs$correlation, 1)
full.subs$rsgn <- ifelse(full.subs$r >= 0, 1, 0)
full.subs$rabs <- abs(full.subs$r)
full.subs$plot <- ifelse(full.subs$test_param == 0, "S", "L")
full.subs <- select(full.subs, subj_id, correct, r, rsgn, rabs, smoothed, n, plot, log_time, response_0, actual_location, pic_id, choice_reason)#[,c(1,7, 15,16,17,20:26)]
full.subs$attempts <- sapply(strsplit(as.character(full.subs$response_0), ","), length)
full.subs$weights = 1/(full.subs$attempts) #weights are actually number of attempts in the glm
full.sub <- full.subs %>% transform(response_0= strsplit(as.character(response_0), ",")) %>% unnest(response_0)
...
subsetted <- select(full.sub, rabs, n, alpha, rsgn, smoothed, plot, subj_id, weights, correct, log_time)
subsetted$rabs <- as.numeric(subsetted$rabs)
subsetted$subj_id <- as.numeric(subsetted$subj_id)
subsetted <- mutate(subsetted, plot = ifelse(plot=="S", 1, 0))
...
#kable(powercoef[7:12,], format="latex", digits=2)
@
\end{table}

Table \ref{tab1} summarises the model estimates. It can be confusing the interpret the model estimates, and at first glance it appears the negative plot coefficient implies that overliad lines are substantially more powerful than the scatter plot, but this is misleading and interpreting the full model involves combining the estimates for the different interactions with abs($\rho$) and sign($\rho$). abs($\rho$) models the power difference between low and high correlations, and abs($\rho)*sign($\rho$) models an increase in power appearing in positive but not negative correlation. 
Taking these into account, the scatter plot shows superior performance and higher values of abs($\rho$), It needs to be restated that we only tested correlations at  0.3, 0.5, 0.7 and 0.9, so the results below an absolute value of 0.3 are unreliable and may be artifacts of regression smoothing methods.
...
\begin{figure}[htbp]
 \centering
<<powermodel, dependson='powerfit', fig.height=8, fig.width=8, out.width='0.7\\textwidth', echo=FALSE, error=FALSE, message=FALSE, warning=FALSE>>=
...
colnames(prediction) <- c("rabs", "t", "alpha", "rsgn", "smoothed", "plot", "subj_id", "weights", "detected", "log_time", "RandomEf", "FixedEf", "residual")
glmm.resid <- subset(prediction, detected==0 | detected==1)
prediction <- mutate(prediction, r = ifelse(rsgn==1, rabs, -rabs), plot = ifelse(plot==0, "Overlaid Lines", "Scatterplot"), plot = ifelse(smoothed==1, paste(plot, "(Smoothed)", sep=" "), plot),  diff = round(RandomEf - FixedEf,5))
randomeffects <- summarise(group_by(prediction, subj_id), re=max(diff)) #all values of diff are identical for a given subject, max chosen just as a function to select one of them
prediction <- select(prediction, r, t, rsgn, smoothed, plot, subj_id, RandomEf, FixedEf)
...
\begin{figure}
  \centering
<<residuals, dependson='powermodel', fig.height=4, fig.width=6, out.width='0.7\\textwidth', echo=FALSE, warning=FALSE, message=FALSE, error=FALSE>>=
...
\begin{figure}[htbp]
  \centering
<<random, dependson='powerfit', fig.height=4, fig.width=6, out.width='0.7\\textwidth', echo=FALSE, warning=FALSE, message=FALSE, error=FALSE>>=
ggplot(data=randomeffects) + geom_histogram(aes(x=re, y=..density..), binwidth=0.08) + xlab("Random Effects") + ylab(NULL)
@
  \label{random}
  \caption{Histograms of estimated subject random effects, $\hat{\gamma}$}
\end{figure}
...
rownames(time.fit) <- c("Intercept", "Smoothed", "t", "r", "sign(r)", "r*sign(r)", "detected", "Plot", "Plot*Smoothed", "Plot*t", "Plot*abs(r)", "Plot*sign(r)", "Plot*abs(r)*sign(r)", "Plot*detected")
...
\lab{tab3}
\centering
<<answertables, dependson='setup', echo=FALSE, results='hide', message=FALSE, error=FALSE, warning=FALSE>>=
...
Table~\ref{tab3} contains the true plot detections / attempts for each lineup in the data collection. True plot correlation is on the x axis and t is on the y axis. 

\begin{figure}[htbp]
\lab{map}
\centering
<<map, , dependson='setup', echo=FALSE, results='hide', message=FALSE, error=FALSE, warning=FALSE>>=
location <- read.csv("location.csv")
map.dat <- map_data("world")
ggplot() + geom_polygon(aes(long,lat, group=group), fill="grey65", data=map.dat) + theme_bw() +
  geom_point(data=location, aes(x=lon, y=lat, colour= "blue")) +scale_colour_manual(values=c("#FF5500", "#00FFFF")) + theme(axis.text=element_blank(), axis.title=element_blank(), legend.position="none")
@
\end{figure}

Figure~\ref{map} shows the location of the recruited subjects. The majority reside in the United States but some are spread around other countries in Europe and Asia.
