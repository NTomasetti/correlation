...
ggplot() + geom_polygon(aes(long,lat, group=group), fill="grey65", data=map.dat) + 
  geom_point(data=location, aes(x=lon, y=lat, colour= "blue")) + scale_colour_manual(values=c("#FF5500", "#00FFFF")) + 
  theme_bw() + theme(aspect.ratio=0.67, axis.text=element_blank(), axis.title=element_blank(), legend.position="none")
@
\end{figure}
