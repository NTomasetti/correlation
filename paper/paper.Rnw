...
Where $\hat\pi_{i} = (x_{i}+1)/(n_{i}+1)$, $x$ is the number of times a true lineup was correctly identified and $n$ is the Welch-Satterwaite estimate for the degrees of freedom.
...
prop <- summarise(group_by(prop, smoothed, n, r), l.attempt=sum(ifelse(design=="Overlaid Lines", evals, 0)),
                                                  l.correct=sum(ifelse(design=="Overlaid Lines", total, 0)),
                                                  s.attempt=sum(ifelse(design=="Scatterplot", evals, 0)),
                                                  s.correct=sum(ifelse(design=="Scatterplot", total, 0)))
