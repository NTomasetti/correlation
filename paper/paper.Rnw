...
#library(xtable)
x <- summary(m8l)$coefficients
y <- summary(m8s)$coefficients
colnames(y)[4] <- "p-value"
rownames(y) <- c("Intercept", "abs(r)", "sign(r)", "smooth", "sample size", "Interaction(r+, r-)")
kable(x, format="latex", digits=2)
kable(y, format="latex", digits=2)
@
\end{table}
...
kable(summary(timelmer)$coefficients, format="latex")
@
