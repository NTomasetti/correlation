...
pin <- select(pin, lineup, plot, count, smoothed, true_pos, n, test_param, r, attempt)#pin[,c(1, 2, 3, 12, 13, 15, 11, 20)]
pin$r <- factor(pin$r)
...
\begin{figure}
  \centering
<<residuals, dependson='powerfit', fig.height=4, fig.width=6, out.width='0.7\\textwidth', echo=FALSE, warning=FALSE, message=FALSE, error=FALSE>>=
p1b <- ggplot(data=subset(glmm.resid, plot==1)) + geom_histogram(aes(y=..density.., x=residual)) + labs(title="Scatter plot", x="residual", y=element_blank())
p2b <- ggplot(data=subset(glmm.resid, plot==0)) + geom_histogram(aes(y=..density.., x=residual))+ labs(title="Overlaid Lines", x="residual", y=element_blank())
grid.arrange(p1b, p2b, ncol=2, top = "Residual density plots")
jarque.test(subset(glmm.resid), plot==1)$residual
jarque.test(subset(glmm.resid), plot==0)$residual
@
  \label{residuals}
...
\begin{figure}
  \centering
<<residuals, dependson='powerfit', fig.height=4, fig.width=6, out.width='0.7\\textwidth', echo=FALSE, warning=FALSE, message=FALSE, error=FALSE>>=
ggplot(data=randomeffects) + geom_histogram(aes(x=re, y=..density..)) + xlab("Random Effects") + ylab(NULL)
jarque.test(randomeffects$re)
@
  \label{random}
