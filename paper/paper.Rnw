...
They created a statistically rigorous framework with properties explored primarily in \citet{Majumder:2014} and further in \citet{Hoffman:2014} that allows us to conduct these hypothesis tests with visual inference, thus to some degree allowing the conjoining of EDA with classical inferential statistics.
...
$p_{i}$, the probability that plot $i$ is chosen in the lineup depends not only on the plot design $d$, but also the signal strength, $q_{i}$, of that plot and of every other competing plot in the lineup. It can be defined as some unknown function $f_{i,d} (q_{1},\dots,q_{20})$ \citep{Hoffman:2014}. 
...
pin$choice <- separate(pin, response_0, c("choice", "l1", "l2"), ",", extra="drop")$choice
pin$choice[str_detect(pin$actual_location, pin$response_0)] <- 
  pin$actual_location[str_detect(pin$actual_location, pin$response_0)]
                         #as.integer(pin$response_0)
counts <- data.frame(table(pin$pic_id, pin$choice))
...
ggplot(data=filter(pin, smoothed==0 & design == "Scatterplot")) + geom_errorbar(aes(x=Correlation, ymin=base, ymax=percent, colour=true)) +
...
full.sub$correct.prop <- full.sub$correct/full.sub$attempts #correct.prop - proportion of correct selections, weights - number of attempts

m8l <- glmer(correct.prop~offset(alpha) + rabs*rsgn + smoothed + n + (1|subj_id), data=filter(full.sub, plot == "L"), family="binomial", weights=attempts)
m8s <- glmer(correct.prop~offset(alpha) + rabs*rsgn + smoothed + n + (1|subj_id), data=filter(full.sub, plot == "S"), family="binomial", weights=attempts)
pander(summary(m8l), justify=c('left', 'right', 'right', 'right', 'right', caption="Model fit for overlaid lines"))
pander(summary(m8s), justify=c('left', 'right', 'right', 'right', 'right', caption="Model fit for scatter plots"))
@
...
full.pred <- rbind(full.pred8s, full.pred8l)


#full.pred <- mutate(full.pred2, plot2 = ifelse(substr(plot, 1, 1) =="S", "Scatterplot", "Overlaid Lines"))

qplot(r, fitre, data=full.pred, color=plot, 
      group=interaction(subj_id, plot), geom="line", alpha=I(0.1)) + 
  scale_color_manual("Plot Design", values=c("#33AA33", "#9977EE", "#FF9933", "#00DDAA")) + 
  xlab("Correlation") + ylab("Estimated Power") + labs(title="Predicted Power") +
...
pander(summary(timelmer), justify=c('left', 'right', 'right', 'right', 'right', caption="Model fit for time taken"))
@
