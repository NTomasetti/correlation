...
$$Power_{i} = \psi\left\{ \beta_{i,0} + \beta_{i,1} \cdot |\alpha| + \beta_{i,2}\cdot Sign + \beta_{i,3}\cdot Smoothed + \beta_{i,4}\cdot t + \beta_{i,5}\cdot |\alpha| \cdot Sign + \gamma_{i,1} \cdot subject_{j} \right\}$$

Where $\psi$ is the logit link function, $\psi(x) = \frac{e^{x}}{1+e^{x}}$, $\alpha$ is the true plot correlation, Sign is the direction of $\alpha$, $i$  is the plot design and $j$ is an observer specific random effect.
...
#library(xtable)
kable(summary(m8l)$coefficients, format="latex")
...
xable(summary(timelmer)$coefficients, format="latex")
@
