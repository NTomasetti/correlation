...
library(mnormt)
library(reshape2)
# Convert reshape2 to tidyr, melt to gather
library(nullabor)
library(dplyr)
library(mFilter)
library(grid)
...
correl <- read.csv("correlations.csv")
full <- read.csv("full.csv", row.names=1) 
Turk <- read.csv("turk.csv") 
...
Turk.line <- Turk.line[order(Turk.line$r, Turk.line$n, Turk.line$smoothed),c(10, 13, 16, 19, 20)]
Turk.line <- summarise(group_by(Turk.line, smoothed, n, r), sum(attempt), sum(correct))
Turk.scatter <- Turk.scatter[order(Turk.scatter$r, Turk.scatter$n, Turk.scatter$smoothed),c(10, 13, 16, 19, 20)]
...
ggplot(data=prop) + geom_line(aes(x=r, y=difference), colour="#99CC99", size=2) + 
                    geom_ribbon(aes(x=r, ymin=lower, ymax=upper), colour="#99CC99", fill="#99CC99", alpha=0.2) + 
                    labs(title="Difference in Power", x="Correlation") + facet_grid(n~smoothed)
@
 \label{powerdiff}
...
\begin{figure}[htbp]
  \centering
<<confidence2, dependson='confidence', fig.width=6, fig.height=6, out.width='0.7\\textwidth', echo=FALSE>>=
...
pin <- cSplit(pin, "response_0", direction="long")
counts <- data.frame(table(pin$pic_id, pin$response_0))
counts$Var1 <- as.integer(counts$Var1)
counts$Var2 <- as.integer(counts$Var2)
colnames(counts) <- c("lineup", "plot", "count")
pin <- merge(counts, Turk, by.x="lineup", by.y="pic_id")
pin <- select(pin, lineup, plot.x, count, smoothed, true_pos, n, test_param, r)#pin[,c(1, 2, 3, 12, 13, 15, 11, 20)]
pin$r <- factor(pin$r)
#colnames(pin) <- c("lineup", "plot", "count", "smoothed", "true_pos", "n", "design", "r")
pin <- mutate(pin, design=ifelse(test_param=="scatter", "Scatterplot", "Overlaid Lines"))#, smoothed=ifelse(smoothed=="no", 0, 1))
pin$base <- 0
pin$Correlation <- diag(as.matrix(correl[pin$lineup, (pin$plot+2)]))
pin <- mutate(pin, true = ifelse(plot==true_pos, "True plot", "Null plot"))
pin <- merge(pin, Turk[,c(2, 16)], by.x = "lineup", by.y = "pic_id")
pin <- mutate(pin, percent = count/attempt)
ggplot(data=subset(pin, smoothed==0 & design == "Scatterplot")) + geom_errorbar(aes(x=Correlation, ymin=base, ymax=percent, colour=true)) +
...
ggplot(data=subset(pin, smoothed==1 & design == "Scatterplot")) + geom_errorbar(aes(x=Correlation, ymin=base, ymax=percent, colour=true)) +
...
ggplot(data=subset(pin, smoothed==0 & design == "Overlaid Lines")) + geom_errorbar(aes(x=Correlation, ymin=base, ymax=percent, colour=true)) +
...
ggplot(data=subset(pin, smoothed==1 & design == "Overlaid Lines")) + geom_errorbar(aes(x=Correlation, ymin=base, ymax=percent, colour=true)) +
...
r <- seq(-0.9,0.9,0.02)
n <- c(12, 24, 48, 96)
full.pred8l <- data.frame(expand.grid(r=r, n=n, alpha=log(.05/0.95),
                                      smoothed=c(0,1), 
                                      subj_id=subset(full.sub, plot=="L")$subj_id)) #ranef(m4)[[1]][,1]))
full.pred8l$rsgn <- ifelse(full.pred8l$r > 0, 1, 0)
full.pred8l$rabs <- abs(full.pred8l$r)
full.pred8l$fitre <- predict(m8l, full.pred8l, type="response", re.form=~(1|subj_id),
                             allow.new.levels=TRUE)
full.pred8l$fitfx <- predict(m8l, full.pred8l, type="response", re.form=NA,
                             allow.new.levels=TRUE)
full.pred8l$plot <- ifelse(full.pred8l$smoothed == 0, "Overlaid Lines", "Overlaid Lines (Smoothed)")
full.pred8l <- filter(full.pred8l, smoothed == 0 | smoothed == 1 & n==48 | smoothed ==1 & n==96)

full.pred8s <- data.frame(expand.grid(r=r, n=n, alpha=log(.05/0.95),
                                      smoothed=c(0,1), 
                                      subj_id=subset(full.sub, plot=="S")$subj_id)) #ranef(m4)[[1]][,1]))
full.pred8s$rsgn <- ifelse(full.pred8s$r > 0, 1, 0)
full.pred8s$rabs <- abs(full.pred8s$r)
full.pred8s$fitre <- predict(m8s, full.pred8s, type="response", re.form=~(1|subj_id),
                             allow.new.levels=TRUE)
full.pred8s$fitfx <- predict(m8s, full.pred8s, type="response", re.form=NA,
                             allow.new.levels=TRUE)
full.pred8s$plot <- ifelse(full.pred8s$smoothed == 0, "Scatterplot", "Scatterplot (Smoothed)")
full.pred8s <- filter(full.pred8s, smoothed == 0 | smoothed == 1 & n==48 | smoothed ==1 & n==96)
full.pred2 <- rbind(full.pred8s, full.pred8l)


full.pred2 <- mutate(full.pred2, plot2 = ifelse(substr(plot, 1, 1) =="S", "Scatterplot", "Overlaid Lines"))

qplot(r, fitre, data=full.pred2, color=plot, 
      group=interaction(subj_id, plot), geom="line", alpha=I(0.1)) + 
  scale_color_manual("Plot Design", values=c("#33AA33", "#9977EE", "#FF9933", "#00DDAA")) + 
  xlab("Correlation") + ylab("Percent Detected") + labs(title="Predicted Power") +
  geom_line(aes(y=fitfx), size=2, alpha=1) + theme_bw() + facet_wrap(~n, ncol = 2) + theme(aspect.ratio=1.2)
@
  \label{powermodel}
...
\begin{figure}
  \centering
<<residuals, dependson='powerfit', fig.height=4, fig.width=6, out.width='0.7\\textwidth', echo=FALSE>>=
resids <- resid(m8s)
residl <- resid(m8l)
resids[842:843] <- NA
...
$$Power_{UR,s} = \psi\left\{ \beta_{s,0} + \beta_{s,1} \cdot |Correlation| + \beta_{s,2}\cdot Sign + \beta_{s,3}\cdot Smoothed + \beta_{s,4}\cdot t + \beta_{s,5}\cdot |Correlation| \cdot Sign + \gamma_{s,1} \cdot subject_{j} \right\}$$
...
$$Power_{R,s} = \psi\left\{ \beta_{s,0}+\beta_{s,1}\cdot |Correlation| + \beta_{s,3}\cdot Smoothed + \beta_{s,4}\cdot t +\gamma_{s,1} \cdot subject_{j} \right\}$$
...
timelmer <-  lmer(log_time ~ smoothed + r + n + rsgn*r + test_param + test_param*smoothed 
                  + test_param*r + test_param*n + test_param*rsgn*r + test_param*correct + (1|subj_id), data=full.sub)
@
%get table code
