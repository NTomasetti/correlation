...
#pred <- subset(pred, smoothed==0 | smoothed == 1 & n ==48 | smoothed ==1 & n ==96)
...
colnames(prediction) <- c("rabs", "t", "alpha", "rsgn", "smoothed", "plot", "subj_id", "weights", "detected", "Included", "RandomEf", "FixedEf", "residual")
glmm.resid <- subset(prediction, detected==0 | detected==1)
prediction <- mutate(prediction, r = ifelse(rsgn==1, rabs, -rabs), plot = ifelse(plot==0, "Overlaid Lines", "Scatterplot"), plot = ifelse(smoothed==1, paste(plot, "(Smoothed)", sep=" "), plot))
...
@
  \label{residuals}
  \caption{Histograms of residuals for both the scatter plot (left) and the overlaid lines (right). Both appear to be normal}
\end{figure}
...
kable(time.fit[1:14,], format="latex", caption="Main Effects", digits=2)
