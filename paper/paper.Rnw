...
#weirw.csv(full.sub, "stata.csv") #exporting data to other programs
...
#library(xtable)
line.fit <- summary(m8l)$coefficients
scatter.fit <- summary(m8s)$coefficients
colnames(scatter.fit)[4] <- "p-value"
rownames(scatter.fit) <- c("Intercept", "abs(r)", "sign(r)", "smoothed", "t", "Interaction(r+, r-)")
#colnames(line.fit)[4] <- "p-value"
#rownames(line.fit) <- c("Intercept", "abs(r)", "sign(r)", "smooth", "sample size", "Interaction(r+, r-)")
kable(scatter.fit, format="latex", digits=2, caption="Scatter plot")
kable(line.fit, format="latex", digits=2, caption="Overlaid Lines")
@
\end{table}
...
r <- seq(-0.9,0.9,0.02)
n <- c(12, 24, 48, 96)
predl <- data.frame(expand.grid(r=r, n=n, alpha=log(.05/0.95),
                                      smoothed=c(0,1), 
                                      subj_id=subset(full.sub, plot=="L")$subj_id)) #ranef(m4)[[1]][,1]))
predl$rsgn <- ifelse(predl$r > 0, 1, 0)
predl$rabs <- abs(predl$r)
predl$fitre <- predict(m8l, predl, type="response", re.form=~(1|subj_id),
                             allow.new.levels=TRUE)
predl$fitfx <- predict(m8l, predl, type="response", re.form=NA,
                             allow.new.levels=TRUE)
predl$plot <- ifelse(predl$smoothed == 0, "Overlaid Lines", "Overlaid Lines (Smoothed)")
predl <- filter(predl, smoothed == 0 | smoothed == 1 & n==48 | smoothed ==1 & n==96)

preds <- data.frame(expand.grid(r=r, n=n, alpha=log(.05/0.95),
                                      smoothed=c(0,1), 
                                      subj_id=subset(full.sub, plot=="S")$subj_id)) #ranef(m4)[[1]][,1]))
preds$rsgn <- ifelse(preds$r > 0, 1, 0)
preds$rabs <- abs(preds$r)
preds$fitre <- predict(m8s, preds, type="response", re.form=~(1|subj_id),
                             allow.new.levels=TRUE)
preds$fitfx <- predict(m8s, preds, type="response", re.form=NA,
                             allow.new.levels=TRUE)
preds$plot <- ifelse(preds$smoothed == 0, "Scatterplot", "Scatterplot (Smoothed)")
preds <- filter(preds, smoothed == 0 | smoothed == 1 & n==48 | smoothed ==1 & n==96)
full.pred <- rbind(preds, predl)
...
\label{tab2}
\centering
\caption{Mixed Linear Model fit for time taken to make a selection}
...
kable(time.fit[1:7,], format="latex", caption="Main Effects (Overlaid Lines)")
kable(time.fit[8:14,], format="latex", caption="Interaction Effects (Scatter plot)")
@
\end{table}
